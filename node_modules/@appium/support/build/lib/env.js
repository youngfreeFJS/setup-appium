...
async (cwd = process.cwd()) => {
    ...
    const readPkg = async (cwd) => {
        ...
        if (pkgPath) {
            logger_1.default.debug(`Found package.json having current Appium dep in ${pkgPath}`);
        }
        else {
            logger_1.default.debug(`No package.json having current Appium dep in ${cwd}`);
        }
        return pkgPath;
    };
    ...
    try {
        ...
        if (pkgDir !== cwd) {
            if (pkgDir) {
                logger_1.default.debug(`Determined package/workspace root from ${cwd} => ${pkgDir}`);
            }
            else {
                pkgDir = cwd;
            }
...
async function _resolveAppiumHome(cwd = process.cwd()) {
    ...
    if (process.env.APPIUM_HOME) {
        logger_1.default.debug(`Using APPIUM_HOME from env: ${process.env.APPIUM_HOME}`);
    ...
    logger_1.default.debug(`Using default APPIUM_HOME: ${exports.DEFAULT_APPIUM_HOME}`);
